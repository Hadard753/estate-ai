...
COPY ./estateai-client/package.json ./estateai-client/package.json
...
RUN chmod +x ./build.sh && npm run build \
    ...
    && cd ./estateai-client && npm prune --production && cd ../ \
    # Copy required node modules
    && cp -Rf ./node_modules ${workdir} \
    && cp -Rf ./estateai-client/node_modules/* ${workdir}/node_modules/ \
